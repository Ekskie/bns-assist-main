...
export async function GET() {
  ...
  try {
    // Fetch all users
    const users = await BnsUser.find().lean();

    // Attach diaries to each user (empty if none)
    const usersWithDiaries = await Promise.all(
      users.map(async (user) => {
        const diaries = await BnsUserDiary.find({ userId: user._id }).lean();

        return {
          ...user,
          diaries: diaries || [], // 🔥 always return an array
        };
      })
    );
